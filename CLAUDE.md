...
- GUI が積んでよいのは semantic 判断を伴わない再実行（finalize / repair）だけ。GUI が触るのは自分の D1 の requests 行のみで、ComfyUI へは到達しない（`docs/worker-protocol.md`）。Compare は semantic metadata の diff 表示まで（指示テキスト生成はしない）

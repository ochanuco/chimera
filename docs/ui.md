...
行内で最初に実値を持つ列を基準（base）とし、各セルは自分自身の値だけを表示した
うえで差分部分だけをハイライトします（WinMerge風のレーン単位ハイライト。他列の
テキストを埋め込むことはしません）。基準セルは、他のいずれかの列との比較で失わ
れているトークンを赤でハイライトします。基準以外の実値セルは、基準に対して自分
だけが持つトークンを緑でハイライトします。strengths / defects / 配列形式の
attributesは項目単位で同じ扱いをし、1行1項目で表示します。`(not analyzed)`、
`—` のセル、行内の実値が1個以下の場合、および差分が無いセルはdiff装飾なしの

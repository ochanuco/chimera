...
-   MCP / GUI から直接書かれた Observation の `id` は `idempotency_key` から作ります。
    内容から作ると、import 側と同じ理由で再測定が黙って消えます。同じ観測を測り直して
    同じ結果が出たら独立した行になるべきで、区別するのは呼び出し側が渡す key です。
    再送は同じ key なら同じ行を返します。
